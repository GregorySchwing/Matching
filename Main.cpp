...
int main(int argc, char **argv) {
    ...
    for (int seed = 0; seed < numSeeds; seed++){
        auto shuffle_start = high_resolution_clock::now();
        Graph G2(G,seed);
        auto shuffle_end = high_resolution_clock::now();
        auto duration_shuffle = duration_cast<milliseconds>(shuffle_end - shuffle_start);
        std::cout << "Shuffle G time: "<< duration_alloc.count() << " milliseconds" << '\n';
        auto match_start = high_resolution_clock::now();
        Matcher::match<int64_t, std::string>(G2);
        ...
        auto count = std::count_if(G2.matching.begin(), G2.matching.end(),[&](auto const& val){ return val > -1; });
        std::cout << "Maximum matching size: "<<  count/2 << '\n';
        std::vector<int64_t> match_count(G2.getM(),0);
        // Iterate through the matching vector and update the match_count array
        for (auto const& val : G2.matching) {
